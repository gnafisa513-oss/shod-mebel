...
// APP STATE
const state = {
    products: safeLoadJSON('shod_products', PRODUCTS),
...
function saveState() {
    try {
        ...
        localStorage.setItem('shod_products', JSON.stringify(state.products));
    } catch (e) {
...
function renderProducts() {
            ...
            <div class="product-card" data-id="${p.id}">
                ...
                <div class="product-info">
                    ...
                    <div class="product-card-actions">
                        ...
                        ${isAdmin ? `<button data-admin-delete-id="${p.id}" class="admin-delete-product" title="Mebelni o'chirish" aria-label="Mebelni o'chirish"><i data-lucide="trash-2"></i></button>` : ''}
                    </div>
                </div>
            </div>
        `;
    ...
    grid.querySelectorAll('[data-admin-delete-id]').forEach(btn => {
        btn.addEventListener('click', (e) => {
            e.stopPropagation();
            const id = parseInt(btn.getAttribute('data-admin-delete-id'));
            const product = state.products.find(p => p.id === id);
            if (!product || !isAdmin) return;
            if (!confirm(`"${product.name || product.title}" mebelini o'chirasizmi?`)) return;
            state.products = state.products.filter(p => p.id !== id);
            state.favorites = state.favorites.filter(favId => favId !== id);
            saveState();
            if (tg?.sendData) {
                try { tg.sendData(JSON.stringify({ action: 'delete_product', product_id: id })); } catch (err) { }
            }
            renderProducts();
            renderFavorites();
            toast("Mebel o'chirildi.", 'success');
        });
    });
}
        ...
        if (tg?.sendData) {
            try {
                tg.sendData(JSON.stringify({
                    action: 'add_product',
                    product: {
                        title, category: categoryInput, price, description: desc,
                        media_url: mediaUrl, is_custom: isCustom
                    }
                }));
            } catch (err) { console.warn('add_product sendData xatosi:', err); }
        }
        form.reset();
        closeModal('adminProductModalOverlay');
        renderCategories();
        renderProducts();
        toast("Mebel qo'shildi va saqlandi.", 'success', 3200);
    });
}
